...
pub async fn run_dashboard(ollama_url: &str) -> Result<()> {
    ...
    loop {
        ...
        if let Ok(res) = result_rx.try_recv() {
            ...
            // Code mode: tool calling handles file creation via orchestrator
            d.status = format!(
                "✓ {} | {} | {:.1} tok/s | {}ms | {} out | {} in",
...
async fn execute_action(d: &mut Dashboard, tx: mpsc::Sender<DashboardResult>, err_tx: mpsc::Sender<(String, u32)>) {
    ...
    match mode {
        ...
        Mode::Code | Mode::Review | Mode::Test | Mode::Refactor | Mode::Autonomous => {
            ...
            tokio::spawn(async move {
                let req = CodeRequest { task: prompt.clone(), language: None, files: Vec::new(), model_override: Some(model.clone()), permission_mode: "bypass".to_string(), max_rounds: 5 };
