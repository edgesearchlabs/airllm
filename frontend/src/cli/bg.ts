...
export async function logsHandler(
  ...
  if (!parsed.target) fail('Usage: openairllm logs <id-or-name> [-f]')
...
export async function attachHandler(
  ...
  if (!target) fail('Usage: openairllm attach <id-or-name>')

  await refreshBackgroundSessionStatuses()
  const session = await resolveSessionOrExit(target)
  console.error(
    `Attach is not implemented for local background sessions yet. Use \`openairllm logs ${session.id} -f\` to follow output.`,
  )
  process.exitCode = 1
}
...
export async function killHandler(
  ...
  const target = normalizeArgs(args)[0]
  if (!target) fail('Usage: openairllm kill <id-or-name>')
...
export async function handleBgFlag(args: string[]): Promise<void> {
  ...
  if (!parsed.prompt && !hasResumeSource(parsed.childArgs)) {
    fail('Usage: openairllm --bg [--name <name>] "<prompt>"')
  }
  ...
  if (!entrypoint) {
    fail('Cannot determine OpenAirLLM entrypoint for background session')
  }
  ...
  console.log(`Follow: openairllm logs ${session.id} -f`)
  console.log(
