...
describe('background session registry', () => {
  ...
  beforeEach(async () => {
    configDir = await mkdtemp(join(tmpdir(), 'openairllm-bg-registry-'))
  ...
  it('creates session metadata and log files under the OpenAirLLM config dir', async () => {
    const session = await createBackgroundSession({
      id: 'bg-test-1',
      name: 'auth-refactor',
      pid: 12345,
      cwd: '/repo',
      command: ['openairllm', '--print', 'refactor auth'],
      provider: 'openai',
      model: 'gpt-5',
  ...
  it('resolves exact live names before session id prefixes', async () => {
    ...
    await createBackgroundSession({
      ...
      pid: 222,
      cwd: '/repo',
      command: ['openairllm', '--print', 'named'],
  ...
  it('rejects duplicate terminal session names as ambiguous', async () => {
    await createBackgroundSession({
      ...
      pid: 111,
      cwd: '/repo',
      command: ['openairllm', '--print', 'old-one'],
    ...
    await createBackgroundSession({
      ...
      pid: 222,
      cwd: '/repo',
      command: ['openairllm', '--print', 'old-two'],
  ...
  it('rejects duplicate live names before considering id prefixes', async () => {
    ...
    await createBackgroundSession({
      ...
      pid: 333,
      cwd: '/repo',
      command: ['openairllm', '--print', 'prefix'],
  ...
  it('does not steal an in-flight name reservation from a live creator', async () => {
    ...
    await expect(
      createBackgroundSession({
        ...
        pid: 222,
        cwd: '/repo',
        command: ['openairllm', '--print', 'contender'],
  ...
  it('recovers orphaned name reservations whose owner metadata is missing', async () => {
    ...
    const session = await createBackgroundSession({
      ...
      pid: 222,
      cwd: '/repo',
      command: ['openairllm', '--print', 'recovered'],
  ...
  it('rejects non-positive pids at creation', async () => {
    ...
    await expect(
      createBackgroundSession({
        ...
        pid: -1,
        cwd: '/repo',
        command: ['openairllm', '--print', 'negative'],
  ...
  it('keeps running sessions fresh when their process identity still matches', async () => {
    await createBackgroundSession({
      ...
      pid: 333,
      cwd: '/repo',
      command: ['openairllm', '--session-id', 'conversation-1', '--print', 'work'],
      sessionId: 'conversation-1',
      now: new Date('2026-06-15T08:00:00.000Z'),
    })
    ...
    const refreshed = await refreshBackgroundSessionStatuses({
      isProcessAlive: () => true,
      getProcessCommand: () =>
        'node openairllm --session-id conversation-1 --print work',
      now: new Date('2026-06-15T08:05:00.000Z'),
    })
  ...
  it('keeps PR-resume sessions fresh when the live command matches the stored invocation', async () => {
    await createBackgroundSession({
      ...
      pid: 333,
      cwd: '/repo',
      command: ['openairllm', '--from-pr', '1642', '--print'],
    ...
    const refreshed = await refreshBackgroundSessionStatuses({
      isProcessAlive: () => true,
      getProcessCommand: () => 'node openairllm --from-pr 1642 --print',
      now: new Date('2026-06-15T08:05:00.000Z'),
    })
