/**
 * `openairllm auth xai ...` command handlers.
 *
...
function listenForManualCode(onLine: (line: string) => void): () => void {
  ...
  // (`openairllm auth xai login`) stays alive after a successful
...
export async function xaiLogout(deps?: XaiLogoutDeps): Promise<void> {
  ...
  // xAI OAuth interactively and then ran `openairllm auth xai logout`
  // ends up in a clean state. Without all four steps, the
  // marker-tagged .openairllm-profile.json survives, startup
