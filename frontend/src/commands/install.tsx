...
export function getInstallationPath(): string {
  ...
  if (isWindows) {
    ...
    const windowsPath = join(homeDir, '.local', 'bin', 'openairllm.exe');
    // Replace forward slashes with backslashes for Windows display
    return windowsPath.replace(/\//g, '\\');
  }
  return '~/.local/bin/openairllm';
}
...
export function Install({
  ...
  useEffect(() => {
    async function run() {
      try {
        ...
        // npm uninstall owns its bin entries and can remove ~/.local/bin/openairllm
  ...
  useEffect(() => {
    if (state.type === 'success') {
      ...
      setTimeout(onDone, 2000, 'OpenAirLLM installation completed successfully', {
        display: 'system' as const
      });
    } else if (state.type === 'error') {
      // Give error message time to render before exiting
      setTimeout(onDone, 3000, 'OpenAirLLM installation failed', {
        display: 'system' as const
      });
    }
  }, [state, onDone]);
  return <Box flexDirection="column" marginTop={1}>
      ...
      {state.type === 'installing' && <Text color="claude">
          Installing OpenAirLLM native build {state.version}...
        </Text>}
      ...
      {state.type === 'success' && <Box flexDirection="column" gap={1}>
          <Box>
            ...
            <Text color="success" bold>
              OpenAirLLM successfully installed!
            </Text>
          </Box>
...
export const install = {
  ...
  name: 'install',
  description: 'Install OpenAirLLM native build',
