...
function Update({ onDone, force, target }: UpdateProps): React.ReactNode {
  ...
  return (
    <Box flexDirection="column" marginTop={1}>
      ...
      {state.type === 'package-manager' && (
        <Box flexDirection="column" gap={1}>
          <Box>
            <StatusIcon status="warning" withSpace />
            <Text color="warning">
              OpenAirLLM is managed by a package manager ({state.manager}).
            </Text>
          </Box>
          <Box marginLeft={2}>
            <Text dimColor>
      ...
      {state.type === 'updating' && (
        <Text color="claude">
          Updating OpenAirLLM to {state.version} via {state.via} (this may take a
          moment)...
        </Text>
      )}
        ...
        <Box flexDirection="column" gap={1}>
          <Box>
            ...
            <Text color="success" bold>
              OpenAirLLM updated to {state.version} via {state.via}!
            </Text>
          </Box>
          <Box marginLeft={2}>
            <Text dimColor>
              Restart OpenAirLLM for the new version to take effect.
            </Text>
          </Box>
        </Box>
      )}
...
function terminalDoneMessage(state: UpdateState): {
  ...
  switch (state.type) {
    case 'success':
      return { message: 'OpenAirLLM updated successfully', delay: 3000 }
    case 'up-to-date':
      return { message: 'OpenAirLLM is already up to date', delay: 1500 }
    case 'blocked':
      return { message: 'Auto-update is unavailable for this build', delay: 3000 }
    case 'package-manager':
      return {
        message: 'OpenAirLLM is managed by a package manager',
        delay: 3000,
      }
    case 'no-package-manager':
      return { message: 'No supported package manager found', delay: 3000 }
    case 'error':
      return { message: 'OpenAirLLM update failed', delay: 4000 }
    default:
