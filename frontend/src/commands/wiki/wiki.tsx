...
Manage the OpenAirLLM project wiki stored in .openairllm/wiki.
...
function formatInitResult(result: Awaited<ReturnType<typeof initializeWiki>>): string {
  const lines = [`Initialized OpenAirLLM wiki at ${result.root}`]
...
function formatStatus(status: Awaited<ReturnType<typeof getWikiStatus>>): string {
  if (!status.initialized) {
    return `OpenAirLLM wiki is not initialized in this project.\n\nRun /wiki init to create ${status.root}.`
  }

  const lines = [
    'OpenAirLLM wiki status',
    '',
    `Root: ${status.root}`,
...
function formatIngestResult(
  ...
  return [
    `Ingested ${result.sourceFile} into the OpenAirLLM wiki.`,
    '',
    `Title: ${result.title}`,
