...
export function AutoUpdater({
  ...
  return <Box flexDirection="row" gap={1}>
      ...
      {(autoUpdaterResult?.status === 'install_failed' || autoUpdaterResult?.status === 'no_permissions') && <Text color="error" wrap="truncate">
          ✗ Auto-update failed &middot; Try <Text bold>openairllm doctor</Text> or{' '}
          <Text bold>
            {hasLocalInstall ? `cd ~/.openairllm/local && npm update ${MACRO.PACKAGE_URL}` : `npm i -g ${MACRO.PACKAGE_URL}@latest`}
          </Text>
        </Text>}
    </Box>;
}
