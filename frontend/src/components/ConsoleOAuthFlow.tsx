    ...
    try {
      ...
      if (mode === 'setup-token') {
        ...
        void sendNotification({
          message: 'OpenAirLLM login successful',
...
function OAuthStatusMessage({
  ...
  switch (oauthStatus.state) {
    case 'idle': {
      const promptText =
        startingMessage ||
        'OpenAirLLM can be used with your Claude subscription or billed based on API usage through your Console account.'
    ...
    case 'creating_api_key':
      return (
        <Box flexDirection="column" gap={1}>
          <Box>
            <Spinner />
            <Text>Creating API key for OpenAirLLM…</Text>
          </Box>
        </Box>
      )
