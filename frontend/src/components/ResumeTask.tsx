  ...
  if (loading) {
    return <Box flexDirection="column" padding={1}>
        <Box flexDirection="row">
          <Spinner />
          <Text bold>Loading OpenAirLLM sessions…</Text>
        </Box>
        <Text dimColor>
          {retrying ? 'Retrying…' : 'Fetching your OpenAirLLM sessions…'}
        </Text>
      </Box>;
  }
  if (loadErrorType) {
    return <Box flexDirection="column" padding={1}>
        <Text bold color="error">
          Error loading OpenAirLLM sessions
        </Text>
  ...
  switch (errorType) {
    ...
    case 'other':
      return <Box marginY={1} flexDirection="row">
          <Text dimColor>Sorry, OpenAirLLM encountered an error</Text>
        </Box>;
  }
}
