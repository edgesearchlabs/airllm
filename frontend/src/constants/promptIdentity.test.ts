...
beforeAll(async () => {
  ...
  ;(globalThis as Record<string, unknown>).MACRO = {
    ...
    ISSUES_EXPLAINER:
      'report the issue at https://github.com/Gitlawb/openairllm/issues',
...
test('CLI identity prefixes describe OpenAirLLM instead of Claude Code', () => {
  expect(getCLISyspromptPrefix()).toContain('OpenAirLLM')
  ...
  for (const prefix of CLI_SYSPROMPT_PREFIXES) {
    expect(prefix).toContain('OpenAirLLM')
    expect(prefix).not.toContain('Claude Code')
    expect(prefix).not.toContain("Anthropic's official CLI for Claude")
  }
})

test('simple mode identity describes OpenAirLLM instead of Claude Code', async () => {
  process.env.CLAUDE_CODE_SIMPLE = '1'

  const prompt = await getSystemPrompt([], 'gpt-4o')

  expect(prompt[0]).toContain('OpenAirLLM')
...
test('built-in agent prompts describe OpenAirLLM instead of Claude Code', () => {
  expect(DEFAULT_AGENT_PROMPT).toContain('OpenAirLLM')
  ...
  expect(generalPrompt).toContain('OpenAirLLM')
  ...
  expect(explorePrompt).toContain('OpenAirLLM')
  ...
  expect(guidePrompt).toContain('OpenAirLLM')
  expect(guidePrompt).toContain('You are the OpenAirLLM guide agent.')
  expect(guidePrompt).toContain('**OpenAirLLM** (the CLI tool)')
