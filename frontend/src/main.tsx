...
if (isBeingDebugged()) {
  ...
  // eslint-disable-next-line custom-rules/no-top-level-side-effects
  process.stderr.write('[openairllm] Debugger detected. Attaching a debugger is not supported in external builds. Exiting.\n')
  process.exit(1);
}
  ...
  if (feature('SSH_REMOTE') && _pendingSSH) {
    ...
    if (rawCliArgs[0] === 'ssh' && rawCliArgs[1] && !rawCliArgs[1].startsWith('-')) {
      ...
      if (rest.includes('-p') || rest.includes('--print')) {
        process.stderr.write('Error: headless (-p/--print) mode is not supported with openairllm ssh\n');
...
async function run(): Promise<CommanderCommand> {
  ...
  program.name('openairllm').description(`OpenAirLLM - starts an interactive session by default, use -p/--print for non-interactive output`).argument('[prompt]', 'Your prompt', String)
  ...
  .option('--provider-env-file <path>', 'Load provider environment variables from a file before validation (repeatable; existing values win)', (val: string, prev: string[]) => [...prev, val], [] as string[]).action(async (prompt, options) => {
    ...
    if (prompt === 'code') {
      ...
      console.warn(chalk.yellow('Tip: You can launch OpenAirLLM with just `openairllm`'));
      prompt = undefined;
    }
    ...
    if (options.continue) {
      ...
      if (!targetSessionId) {
        ...
        if (sessions.length === 0) {
          ...
          return await exitWithMessage(root, `Assistant installed in ${installedDir}. The daemon is starting up — run \`openairllm assistant\` again in a few seconds to connect.`, {
            exitCode: 0,
      ...
      if (remote !== null) {
        ...
        if (!isRemoteTuiEnabled) {
          ...
          process.stdout.write(`Resume with: openairllm --teleport ${createdSession.id}\n`);
          await gracefulShutdown(0);
          process.exit(0);
        }
            ...
            if (repoValidation.status === 'mismatch' || repoValidation.status === 'not_in_repo') {
              ...
              if (sessionRepo) {
                ...
                if (existingPaths.length > 0) {
                  ...
                  throw new TeleportOperationError(`You must run openairllm --teleport ${teleport} from a checkout of ${sessionRepo}.`, chalk.red(`You must run openairllm --teleport ${teleport} from a checkout of ${chalk.bold(sessionRepo)}.\n`));
                }
              }
  ...
  }).version(`${MACRO.DISPLAY_VERSION ?? MACRO.VERSION} (OpenAirLLM)`, '-v, --version', 'Output the version number');
  ...
  mcp.command('serve').description(`Start the OpenAirLLM MCP server`).option('-d, --debug', 'Enable debug mode', () => true).option('--verbose', 'Override verbose mode setting from config', () => true).action(async ({
    debug,
    verbose
  }: {
    debug?: boolean;
  ...
  if (feature('DIRECT_CONNECT')) {
    program.command('server').description('Start an OpenAirLLM session server').option('--port <number>', 'HTTP port', '0').option('--host <string>', 'Bind address', '0.0.0.0').option('--auth-token <token>', 'Bearer token for auth').option('--unix <path>', 'Listen on a unix domain socket').option('--workspace <dir>', 'Default working directory for sessions that do not specify cwd').option('--idle-timeout <ms>', 'Idle timeout for detached sessions in ms (0 = never expire)', '600000').option('--max-sessions <n>', 'Maximum concurrent sessions (0 = unlimited)', '32').action(async (opts: {
      ...
      if (existing) {
        process.stderr.write(`An OpenAirLLM server is already running (pid ${existing.pid}) at ${existing.httpUrl}\n`);
        process.exit(1);
      }
  ...
  if (feature('DIRECT_CONNECT')) {
    program.command('open <cc-url>').description('Connect to an OpenAirLLM server (internal — use cc:// URLs)').option('-p, --print [prompt]', 'Print mode (headless)').option('--output-format <format>', 'Output format: text, json, stream-json', 'text').action(async (ccUrl: string, opts: {
  ...
  const pluginCmd = program.command('plugin').alias('plugins').description('Manage OpenAirLLM plugins').configureHelp(createSortedHelpConfig());
  ...
  const marketplaceCmd = pluginCmd.command('marketplace').description('Manage OpenAirLLM marketplaces').configureHelp(createSortedHelpConfig());
  ...
  if (feature('KAIROS')) {
    program.command('assistant [sessionId]').description('Attach the REPL as a client to a running bridge session. Discovers sessions via API if no sessionId given.').action(() => {
      ...
      process.stderr.write('Usage: openairllm assistant [sessionId]\n\n' + 'Attach the REPL as a viewer client to a running bridge session.\n' + 'Omit sessionId to discover and pick from available sessions.\n');
      process.exit(1);
    });
  }
  ...
  const doctorCommand = program
    .command('doctor')
    .description('Check the health of your OpenAirLLM auto-updater. Note: The workspace trust dialog is skipped and stdio servers from .mcp.json are spawned for health checks. Only use this command in directories you trust.');
  doctorCommand
    .command('report')
  ...
  // claude install
  program.command('install [target]').description('Install OpenAirLLM native build. Use [target] to specify version (stable, latest, or specific version)').option('--force', 'Force installation even if already installed').action(async (target: string | undefined, options: {
    force?: boolean;
  }) => {
    const {
      installHandler
