...
export function REPL({
  ...
  const terminalTitle = sessionTitle ?? agentTitle ?? haikuTitle ?? 'OpenAirLLM';
  ...
  useEffect(() => {
    ...
    setMessages(prev => [...prev, createSystemMessage(`Worktree creation took ${secs}s. For large repos, set \`worktree.sparsePaths\` in .openairllm/settings.json to check out only the directories you need — e.g. \`{"worktree": {"sparsePaths": ["src", "packages/foo"]}}\`.`, 'info')]);
  }, [setMessages]);
  ...
  // That write storm is the primary trigger for ~/.openairllm.json corruption
  // (GH #3117).
  ...
  useEffect(() => {
    const handleSuspend = () => {
      ...
      process.stdout.write(`\nOpenAirLLM has been suspended. Run \`fg\` to bring OpenAirLLM back.\nNote: ctrl + z now suspends OpenAirLLM, ctrl + _ undoes input.\n`);
    };
    const handleResume = () => {
