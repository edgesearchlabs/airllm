...
describe('fetchNextTip / confirmTip (mocked fetch)', () => {
  ...
  test('POSTs sanitized conversation context when a prompt is given', async () => {
    ...
    stubFetch(200, TIP)
    const tip = await fetchNextTip('code', 'openairllm', 'deploy with key sk-ABCDEFGH1234567890')
    expect(captured.method).toBe('POST')
    expect(captured.url).toContain('/api/ads/next?surface=openairllm')
  ...
  test('GETs with no body when there is no prompt', async () => {
    stubFetch(200, TIP)
    await fetchNextTip('code', 'openairllm')
  ...
  test('returns null on a non-OK response', async () => {
    ...
    expect(await fetchNextTip('code', 'openairllm', 'a prompt')).toBeNull()
  })
  ...
  test('clamps a malformed dwell_ms to the 5000ms default', async () => {
    ...
    const tip = await fetchNextTip('code', 'openairllm', 'a prompt')
