...
function createTempAuthJson(payload: Record<string, unknown>): string {
  const dir = mkdtempSync(join(tmpdir(), 'openairllm-codex-'))
  tempDirs.push(dir)
...
describe('Codex request translation', () => {
  ...
  test('recovers Codex web search text and sources from sparse completed response', () => {
    const output = webSearchToolTest.makeOutputFromCodexWebSearchResponse(
        ...
        output: [
          {
            ...
            sources: [
              {
                title: 'OpenAirLLM repo',
                url: 'https://github.com/example/openairllm',
              },
            ],
          },
          {
            ...
            content: [
              {
                type: 'text',
                text: 'OpenAirLLM is available on GitHub.',
                sources: [
                  {
                    title: 'Docs',
                    url: 'https://docs.example.com/openairllm',
                  },
                ],
              },
            ],
      ...
      'OpenAirLLM GitHub 2026',
      0.42,
    )

    expect(output.results).toEqual([
      'OpenAirLLM is available on GitHub.',
      {
            ...
            content: [
              {
                ...
                sources: [
                  { title: 'Docs', url: 'https://docs.example.com/openairllm' },
                ],
              },
            ],
          },
  ...
  test('Codex stream: tool args delivered only via function_call_arguments.done (#1259)', async () => {
    const args = '{"path":"./openairllm-codex-repro","pattern":"**/*.md"}'
    const responseText = [
      'event: response.output_item.added',
