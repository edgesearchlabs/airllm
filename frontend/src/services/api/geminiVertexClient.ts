...
// If openairllm passes a tight budget — common for the first turn of a chat —
...
function toGeminiContents(
  ...
  for (const message of messages) {
    ...
    // follow the model's functionCall turn. openairllm, however, appends
...
// openairllm ships a large coding-agent system prompt as params.system. Vertex
...
// sequence the rest of openairllm consumes. Emits one content_block_start /
...
export function createGeminiVertexClient(options: GeminiVertexClientOptions) {
  ...
  const create = (params: MessageCreateParamsBase & { stream?: boolean }): GeminiVertexPromise => {
    ...
    const promise = (async (): Promise<GeminiVertexMessage> => {
      ...
      // emits ZERO text parts (finishReason STOP, empty response). openairllm,
