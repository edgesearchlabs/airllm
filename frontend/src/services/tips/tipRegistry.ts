...
import { getUserSkillExampleDisplayPath } from '../../utils/claudeCodeDisplayPaths.js'
import {
...
const externalTips: Tip[] = [
  {
    id: 'new-user-warmup',
    content: async () =>
      `Start with small features or bug fixes, tell OpenAirLLM to propose a plan, and verify its suggested edits`,
    cooldownSessions: 3,
    async isRelevant() {
  ...
  },
  {
    id: 'git-worktrees',
    content: async () =>
      'Use git worktrees to run multiple OpenAirLLM sessions in parallel.',
    cooldownSessions: 10,
    isRelevant: async () => {
      try {
        const config = getGlobalConfig()
  ...
  },
  {
    ...
    content: async () =>
      'Running multiple OpenAirLLM sessions? Use /color and /rename to tell them apart at a glance.',
    cooldownSessions: 10,
    isRelevant: async () => {
  ...
  {
    id: 'memory-command',
    content: async () => 'Use /memory to view and manage OpenAirLLM memory',
    cooldownSessions: 15,
    async isRelevant() {
      const config = getGlobalConfig()
  ...
  },
  {
    id: 'prompt-queue',
    content: async () =>
      'Hit Enter to queue up additional messages while OpenAirLLM is working.',
    cooldownSessions: 5,
    async isRelevant() {
      const config = getGlobalConfig()
  ...
  },
  {
    ...
    content: async () =>
      'Send messages to OpenAirLLM while it works to steer OpenAirLLM in real-time',
    cooldownSessions: 20,
    isRelevant: async () => true,
  },
  {
    id: 'todo-list',
    content: async () =>
      'Ask OpenAirLLM to create a todo list when working on complex tasks to track progress and remain on track',
    cooldownSessions: 20,
    isRelevant: async () => true,
  },
  {
  ...
  },
  {
    ...
    content: async () => 'Connect OpenAirLLM to your IDE · /ide',
    cooldownSessions: 4,
    async isRelevant() {
  ...
  },
  {
    ...
    content: async () =>
      'Run /install-github-app to enable GitHub issue and PR tagging from OpenAirLLM',
    cooldownSessions: 10,
    isRelevant: async () => !getGlobalConfig().githubActionSetupCount,
  },
  {
    id: 'install-slack-app',
    content: async () => 'Run /install-slack-app to use OpenAirLLM in Slack',
    cooldownSessions: 10,
  ...
  },
  {
    id: 'paste-images-mac',
    content: async () =>
      'Paste images into OpenAirLLM using control+v (not cmd+v!)',
    cooldownSessions: 10,
  ...
  },
  {
    id: 'continue',
    content: async () =>
      'Run openairllm --continue or openairllm --resume to resume a conversation',
    cooldownSessions: 10,
    isRelevant: async () => true,
  },
  {
  ...
  },
  {
    id: 'desktop-app',
    content: async () =>
      'Run OpenAirLLM locally or remotely with /desktop',
    cooldownSessions: 15,
  ...
  },
  {
    ...
    content: async ctx => {
      ...
      return variant === 'copy_b'
        ? `Use ${cmd} for better one-shot answers. OpenAirLLM thinks it through first.`
  ...
  },
  {
    ...
    content: async ctx => {
      ...
      return variant === 'copy_b'
        ? `For big tasks, tell OpenAirLLM to ${blue('use subagents')}. They work in parallel and keep your main thread clean.`
        : `Say ${blue('"fan out subagents"')} and OpenAirLLM sends a team. Each one digs deep so nothing gets missed.`
    },
    cooldownSessions: 3,
    isRelevant: async () => {
      if (!is1PApiCustomer()) return false
  ...
  },
  {
    ...
    content: async ctx => {
      ...
      return reward
        ? `Share OpenAirLLM and earn ${claude(formatCreditAmount(reward))} of extra usage · ${claude('/passes')}`
