...
async function makeProjectDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'openairllm-wiki-init-'))
  tempDirs.push(dir)
  return dir
}
...
test('initializeWiki creates the expected wiki scaffold', async () => {
  ...
  expect(result.createdFiles).toEqual([
    join('.openairllm', 'wiki', 'schema.md'),
    join('.openairllm', 'wiki', 'index.md'),
    join('.openairllm', 'wiki', 'log.md'),
    join('.openairllm', 'wiki', 'pages', 'architecture.md'),
    join('.openairllm', 'wiki', 'pages', 'conventions.md'),
  ])
  expect(await readFile(paths.schemaFile, 'utf8')).toContain(
    '# OpenAirLLM Wiki Schema',
  )
  expect(await readFile(paths.indexFile, 'utf8')).toContain('Wiki')
  expect(await readFile(paths.logFile, 'utf8')).toContain(
    'Wiki initialized by OpenAirLLM',
  )
