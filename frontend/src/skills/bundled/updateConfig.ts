...
| \`.openairllm/settings.json\` | Project | Commit | Team-wide hooks, permissions, plugins |
| \`.openairllm/settings.local.json\` | Project | Gitignore | Personal overrides for this project |
...
{
  "permissions": {
    "allow": ["Bash(npm:*)", "Edit(.openairllm)", "Read"],
    "deny": ["Bash(rm -rf:*)"],
    "ask": ["Write(/etc/*)"],
...
4. **Write the JSON.** Merge into the target file (schema shape in the "Hook Structure" section above). If this creates \`.openairllm/settings.local.json\` for the first time, add it to .gitignore — the Write tool doesn't auto-gitignore it.
...
6. **Prove the hook fires** — only for \`Pre|PostToolUse\` on a matcher you can trigger in-turn (\`Write|Edit\` via Edit, \`Bash\` via Bash). \`Stop\`/\`UserPromptSubmit\`/\`SessionStart\` fire outside this turn — skip to step 7.
   ...
   **If proof fails but pipe-test passed and \`jq -e\` passed**: the settings watcher isn't watching \`.openairllm/\` — it only watches directories that had a settings file when this session started. The hook is written correctly. Tell the user to open \`/hooks\` once (reloads config) or restart — you can't do this yourself; \`/hooks\` is a user UI menu and opening it ends this turn.
...
2. **Read**: \`.openairllm/settings.json\` (or create if missing)
...
1. **Check the settings file** - Read ${USER_SETTINGS_PATH_TOKEN} or .openairllm/settings.json
2. **Verify JSON syntax** - Invalid JSON silently fails
3. **Check the matcher** - Does it match the tool name? (e.g., "Bash", "Write", "Edit")
4. **Check hook type** - Is it "command", "prompt", or "agent"?
5. **Test the command** - Run the hook command manually to see if it works
6. **Use --debug** - Run \`openairllm --debug\` to see hook execution logs
`
