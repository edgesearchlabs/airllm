    ...
    refreshProc.on('close', (code, signal) => {
      if (code === 0) {
        ...
        const message = timedOut
          ...
          : chalk.red(
              'Error running awsAuthRefresh (in settings or ~/.openairllm.json):',
            )
  ...
  try {
    ...
    try {
      ...
      const message = chalk.red(
        'Error getting AWS credentials from awsCredentialExport (in settings or ~/.openairllm.json):',
      )
    ...
    refreshProc.on('close', (code, signal) => {
      if (code === 0) {
        ...
        const message = timedOut
          ...
          : chalk.red(
              'Error running gcpAuthRefresh (in settings or ~/.openairllm.json):',
            )
        // biome-ignore lint/suspicious/noConsole:: intentional console output
        console.error(message)
        authStatusManager.endAuthentication(false)
...
export async function validateForceLoginOrg(): Promise<OrgValidationResult> {
  ...
  // in ~/.openairllm.json is user-writable and cannot be trusted.
  ...
  if (!profile) {
    ...
    return {
      ...
      message:
        ...
        `verification (tokens from 'openairllm setup-token' do not include this scope).\n` +
        `Try again, or obtain a full-scope token via 'openairllm auth login'.`,
    }
  }
  ...
  return {
    ...
    message:
      ...
      `Please log in with the correct organization: openairllm auth login`,
  }
}
