...
async function writeJsonl(entry: unknown): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'openairllm-conversation-recovery-'))
  tempDirs.push(dir)
...
test('findResumeLogByPrSelector selects the first non-sidechain PR match', async () => {
  ...
  const linked = {
    ...
    sessionId: id(12),
    prNumber: 1642,
    prUrl: 'https://github.com/Gitlawb/openairllm/pull/1642',
    prRepository: 'Gitlawb/openairllm',
  } as any
  const sidechain = {
    ...linked,
    isSidechain: true,
  ...
  const unrelated = {
    ...
    sessionId: id(14),
    prNumber: 17,
    prUrl: 'https://github.com/Gitlawb/openairllm/pull/17',
  } as any
  ...
  expect(
    findResumeLogByPrSelector(
      ...
      'https://github.com/Gitlawb/openairllm/pull/1642',
    ),
  ).toBe(linked)
  expect(
