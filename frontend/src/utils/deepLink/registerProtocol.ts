...
export function getProtocolBinaryName(platform = process.platform): string {
  const baseName =
    MACRO.PACKAGE_URL === '@anthropic-ai/claude-code' ? 'claude' : 'openairllm'
 ...
 * a cached flag in ~/.openairllm.json, so:
...
export async function ensureDeepLinkProtocolRegistered(): Promise<void> {
  ...
  // ~/.claude (per-machine, not synced) rather than ~/.openairllm.json (can sync).
