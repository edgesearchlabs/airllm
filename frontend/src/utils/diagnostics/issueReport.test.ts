...
describe('diagnostic issue report', () => {
  test('builds a safe JSON report without secrets or full home paths', async () => {
    const report = await buildIssueReport({
      env: baseEnv,
      cwd: '/home/alice/private/openairllm',
      ...
      errors: [
        {
          error:
            'Error: request failed with sk-openai-secret at /home/alice/private/openairllm/src/file.ts',
    ...
    expect(report.openairllm.version).toBe('0.18.0')
    expect(report.workspace.cwd).toBe('openairllm')
  ...
  test('formats markdown suitable for a GitHub issue', async () => {
    ...
    expect(markdown).toContain('# OpenAirLLM diagnostic report')
  ...
  test('falls back safely when build macros are absent in source tests', async () => {
    ...
    try {
      ...
      expect(report.openairllm.version).toBe('unknown')
    } finally {
      if (hadMacro) {
  ...
  test('redacts include-debug error details', async () => {
    ...
    const report = await buildIssueReport({
      ...
      errors: [
        {
          error: `ProviderError: failed with sk-openai-secret-token at ${home}/private/openairllm/src/file.ts`,
          timestamp: '2026-06-15T10:00:00.000Z',
        },
      ],
      includeDebug: true,
    ...
    expect(report.errors.debug).toEqual([
      'ProviderError: failed with [redacted] at ~/private/openairllm/src/file.ts',
    ])
  ...
  test('writes report files and creates parent directories', () => {
    const tempDir = mkdtempSync(join(tmpdir(), 'openairllm-report-'))
    try {
