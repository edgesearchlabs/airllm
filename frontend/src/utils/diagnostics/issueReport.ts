...
function detectSource(cwd: string): IssueReport['openairllm']['source'] {
...
export function formatIssueReportAsMarkdown(report: IssueReport): string {
  const lines = [
    '# OpenAirLLM diagnostic report',
    '',
    '## Summary',
    `- OpenAirLLM: ${report.openairllm.displayVersion ?? report.openairllm.version}`,
