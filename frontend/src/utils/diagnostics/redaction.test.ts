...
describe('diagnostic redaction', () => {
  ...
  test('redacts secret-looking values even under harmless field names', () => {
    ...
    const redacted = redactDiagnosticObject({
      ...
      path: `${home}/private/openairllm/src/file.ts`,
    ...
    expect(redacted.path).toBe('~/private/openairllm/src/file.ts')
  ...
  test('redacts Windows-style home paths without matching sibling directories', () => {
    ...
    expect(
      redactHomePath(
        'debug path C:\\Users\\Alice\\AppData\\Roaming\\openairllm',
        home,
      ),
    ).toBe('debug path ~\\AppData\\Roaming\\openairllm')
    expect(redactHomePath('C:\\Users\\AliceOther\\openairllm', home)).toBe(
      'C:\\Users\\AliceOther\\openairllm',
    )
  })
