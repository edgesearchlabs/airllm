...
export async function detectStaleProjectSettingsPaths(
  ...
  return {
    issue: `Legacy project settings file${stale.length === 1 ? '' : 's'} ${legacyPaths} found, but OpenAirLLM reads ${canonicalPaths}`,
    fix: `Move or copy ${legacyPaths} to ${canonicalPaths} if you intended OpenAirLLM to use those project settings.`,
  }
}
    ...
    if (!claudeInPath && !validAlias) {
      if (existingAlias) {
        ...
        warnings.push({
          ...
          fix: `Alias exists but points to invalid target: ${existingAlias}. Update alias: alias ${getCliBinaryName()}="~/.openairllm/local/${getCliBinaryName()}"`,
        })
      } else {
        // No alias exists and not in PATH
        warnings.push({
          issue: 'Local installation not accessible',
          fix: `Create alias: alias ${getCliBinaryName()}="~/.openairllm/local/${getCliBinaryName()}"`,
        })
      }
    }
  }
