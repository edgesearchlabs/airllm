...
beforeEach(async () => {
  ...
  tempDir = mkdtempSync(join(tmpdir(), 'openairllm-env-test-'))
...
test('getGlobalClaudeFile: new install returns .openairllm.json when neither file exists', async () => {
  const { getGlobalClaudeFile } = await importFreshEnvModule()
  expect(getGlobalClaudeFile()).toBe(join(tempDir, '.openairllm.json'))
})
...
test('getGlobalClaudeFile: migrated user uses .openairllm.json when both files exist', async () => {
  writeFileSync(join(tempDir, '.claude.json'), '{}')
  writeFileSync(join(tempDir, '.openairllm.json'), '{}')
  const { getGlobalClaudeFile } = await importFreshEnvModule()
  expect(getGlobalClaudeFile()).toBe(join(tempDir, '.openairllm.json'))
})

test('getGlobalClaudeFile: OPENCLAUDE_CONFIG_DIR uses preferred config dir', async () => {
  const preferredDir = mkdtempSync(join(tmpdir(), 'openairllm-preferred-env-test-'))
  try {
    ...
    expect(getGlobalClaudeFile()).toBe(join(preferredDir, '.openairllm.json'))
  } finally {
