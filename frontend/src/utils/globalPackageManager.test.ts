...
describe('getGlobalInstallArgs', () => {
  const spec = '@edgesearch/openairllm@latest'
  ...
  test('passes an explicit version spec through unchanged', () => {
    expect(getGlobalInstallArgs('npm', '@edgesearch/openairllm@1.2.3')).toEqual([
      'install',
      '-g',
      '@edgesearch/openairllm@1.2.3',
    ])
  })
})
...
describe('selectOwningPackageManager', () => {
  ...
  test('matches the package manager whose root contains the binary', () => {
    expect(
      selectOwningPackageManager(
        '/home/u/.local/share/pnpm/global/5/node_modules/@edgesearch/openairllm/cli.js',
        [
          { pm: 'npm', root: '/usr/local/lib/node_modules' },
          {
            pm: 'pnpm',
  ...
  test('most specific (longest) root wins when roots are nested', () => {
    ...
    expect(
      selectOwningPackageManager('/opt/pm/node_modules/bun/global/openairllm', [
  ...
  test('does not match a sibling directory sharing a prefix', () => {
    ...
    expect(
      selectOwningPackageManager('/a/node_modules-other/openairllm/cli.js', [
  ...
  test('ignores candidates with an empty root', () => {
    expect(
      selectOwningPackageManager('/usr/lib/node_modules/openairllm', [
        { pm: 'yarn', root: '' },
