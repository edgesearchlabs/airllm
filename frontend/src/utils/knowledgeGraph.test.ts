...
describe('KnowledgeGraph Global Persistence & RAG', () => {
  ...
  beforeEach(async () => {
    ...
    configDir = mkdtempSync(join(tmpdir(), 'openairllm-test-'))
  ...
  it('persists entities across loads', async () => {
    await addGlobalEntity('tool', 'openairllm', { status: 'alpha' })
    ...
    const entities = Object.values(graph.entities).filter(e => e.name === 'openairllm')
  ...
  it('deduplicates entities and updates attributes', async () => {
    await addGlobalEntity('tool', 'openairllm', { status: 'alpha' })
    await addGlobalEntity('tool', 'openairllm', { status: 'beta', version: '0.6.0' })
