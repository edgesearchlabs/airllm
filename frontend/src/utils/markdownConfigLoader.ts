...
const PROJECT_CONFIG_DIR_NAMES = ['.claude', '.openairllm'] as const
...
// ~/.openairllm/agents — see issue #769) opens that many fds and blocks the
...
function recordOversizedSkip(skip: OversizedMarkdownSkip): void {
  ...
  if (!oversizedSkipStderrWarned) {
    ...
    process.stderr.write(
      `openairllm: skipping oversized markdown config file ${skip.filePath} ` +
