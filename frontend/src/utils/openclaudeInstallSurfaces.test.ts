...
beforeEach(async () => {
  await acquireSharedMutationLock('utils/openairllmInstallSurfaces.test.ts')
})
...
test('install command displays ~/.local/bin/openairllm on non-Windows', async () => {
  await mockEnvPlatform('darwin')

  const { getInstallationPath } = await importFreshInstallCommand()

  expect(getInstallationPath()).toBe('~/.local/bin/openairllm')
})

test('install command displays openairllm.exe path on Windows', async () => {
  await mockEnvPlatform('win32')

  const { getInstallationPath } = await importFreshInstallCommand()

  expect(getInstallationPath()).toBe(
    join(homedir(), '.local', 'bin', 'openairllm.exe').replace(/\//g, '\\'),
  )
})

test('native installer uses openairllm launcher for OpenAirLLM package', async () => {
  ;(globalThis as Record<string, unknown>).MACRO = {
    PACKAGE_URL: '@edgesearch/openairllm',
  }

  const { getBinaryName, getExecutableName } = await importFreshInstaller()

  expect(getBinaryName('linux-x64')).toBe('claude')
  expect(getExecutableName('linux-x64')).toBe('openairllm')
  expect(getExecutableName('win32-x64')).toBe('openairllm.exe')
})
...
test('deep-link protocol resolver uses openairllm launcher for OpenAirLLM package', async () => {
  ;(globalThis as Record<string, unknown>).MACRO = {
    PACKAGE_URL: '@edgesearch/openairllm',
  }
  ...
  expect(getProtocolBinaryName('linux')).toBe('openairllm')
  expect(getProtocolBinaryName('win32')).toBe('openairllm.exe')
})
    ...
    void render(
      createElement(Install, {
        ...
        onDone: (result: string) => {
          try {
            expect(result).toBe('OpenAirLLM installation completed successfully')
            resolve()
          } catch (error) {
            reject(error)
          }
...
test('cleanupNpmInstallations removes both openairllm and legacy claude local install dirs', async () => {
  ...
  mock.module('./envUtils.js', () => ({
    ...realEnvUtils,
    getClaudeConfigHomeDir: () => join(homedir(), '.openairllm'),
  }))

  const { cleanupNpmInstallations } = await importFreshInstaller()
  await cleanupNpmInstallations()

  expect(removedPaths).toContain(join(homedir(), '.openairllm', 'local'))
  expect(removedPaths).toContain(join(homedir(), '.claude', 'local'))
})

test('cleanupNpmInstallations manual fallback removes openairllm npm shim', async () => {
  await mockEnvPlatform('darwin')

  const testHome = join(process.cwd(), 'work', 'openairllm-install-home-test')
  const npmPrefix = join(testHome, '.npm-global')
  const shimPath = join(npmPrefix, 'bin', 'openairllm')
  ;(globalThis as Record<string, unknown>).MACRO = {
    PACKAGE_URL: '@edgesearch/openairllm',
  }
  process.env.HOME = testHome
  process.env.USERPROFILE = testHome
  process.env.CLAUDE_CONFIG_DIR = join(testHome, '.openairllm')
