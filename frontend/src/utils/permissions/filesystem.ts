 ...
 * .openairllm/skills/{name}/ directory (global), plus the legacy global
...
export function getClaudeSkillScope(
  ...
  const bases = [
    ...
    {
      dir: expandPath(join(homedir(), '.openairllm', 'skills')),
      prefix: '~/.openairllm/skills/',
    },
    {
...
export function isClaudeSettingsPath(filePath: string): boolean {
  ...
  if (
    normalizedPath.endsWith(`${sep}.openairllm${sep}settings.json`) ||
    normalizedPath.endsWith(`${sep}.openairllm${sep}settings.local.json`) ||
...
function isClaudeConfigFilePath(filePath: string): boolean {
  ...
  const openCommandsDir = join(getOriginalCwd(), '.openairllm', 'commands')
  const openAgentsDir = join(getOriginalCwd(), '.openairllm', 'agents')
  const openSkillsDir = join(getOriginalCwd(), '.openairllm', 'skills')
...
export function isOpenAirLLMCommitMessagePath(absolutePath: string): boolean {
...
export function checkWritePermissionForTool<Input extends AnyObject>(
  ...
  if (claudeFolderAllowRule) {
    ...
    // '~/.openairllm/**', and legacy '~/.claude/**') plus narrowed skill
    // patterns like '~/.openairllm/skills/my-skill/**' so users can grant
...
export function checkEditableInternalPath(
  ...
  if (
    ...
    isOpenAirLLMCommitMessagePath(normalizedPath)
  ) {
    return {
      behavior: 'allow',
      updatedInput: input,
      decisionReason: {
        type: 'other',
        reason: 'OpenAirLLM commit message file is allowed for writing',
      },
    }
  }
