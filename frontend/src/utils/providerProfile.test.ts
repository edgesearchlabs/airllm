...
const missingCodexAuthPath = join(tmpdir(), 'openairllm-missing-codex-auth.json')
...
test('codex launch prefers auth account id over stale persisted value', async () => {
  const codexHome = mkdtempSync(join(tmpdir(), 'openairllm-codex-'))
  try {
    writeFileSync(
...
test('saveProfileFile writes a profile that loadProfileFile can read back', () => {
  const cwd = mkdtempSync(join(tmpdir(), 'openairllm-profile-file-'))
...
test('saveProfileFile defaults to user config instead of the working directory', () => {
  const cwd = mkdtempSync(join(tmpdir(), 'openairllm-workspace-profile-'))
  const configRoot = mkdtempSync(join(tmpdir(), 'openairllm-config-profile-'))
...
test('loadProfileFile keeps project-local files as a legacy fallback', () => {
  const cwd = mkdtempSync(join(tmpdir(), 'openairllm-legacy-profile-'))
  const configDir = mkdtempSync(join(tmpdir(), 'openairllm-empty-config-profile-'))
  const previousConfigDir = process.env.CLAUDE_CONFIG_DIR
  const previousCwd = process.cwd()
...
test('loadProfileFile does not fall back when user config profile is invalid', () => {
  const cwd = mkdtempSync(join(tmpdir(), 'openairllm-invalid-profile-'))
  const configDir = mkdtempSync(join(tmpdir(), 'openairllm-invalid-config-profile-'))
  const previousConfigDir = process.env.CLAUDE_CONFIG_DIR
  const previousCwd = process.cwd()
...
test('deleteProfileFile clears the default profile and legacy workspace fallback', () => {
  const cwd = mkdtempSync(join(tmpdir(), 'openairllm-delete-profile-'))
  const configDir = mkdtempSync(join(tmpdir(), 'openairllm-delete-config-profile-'))
  const previousConfigDir = process.env.CLAUDE_CONFIG_DIR
  const previousCwd = process.cwd()
...
test('deleteProfileFile with configDir and cwd clears both user config and legacy fallback', () => {
  const cwd = mkdtempSync(join(tmpdir(), 'openairllm-delete-mixed-profile-'))
  const configDir = mkdtempSync(join(tmpdir(), 'openairllm-delete-mixed-config-profile-'))
  const previousConfigDir = process.env.CLAUDE_CONFIG_DIR
  const previousCwd = process.cwd()
...
test('clearPersistedCodexOAuthProfile removes only persisted Codex OAuth profiles', async () => {
  const cwd = mkdtempSync(join(tmpdir(), 'openairllm-codex-oauth-profile-'))
...
test('clearPersistedCodexOAuthProfile clears both default and legacy OAuth profiles', async () => {
  const cwd = mkdtempSync(join(tmpdir(), 'openairllm-clear-oauth-profile-'))
  const configDir = mkdtempSync(join(tmpdir(), 'openairllm-clear-oauth-config-'))
  const previousConfigDir = process.env.CLAUDE_CONFIG_DIR
  const previousCwd = process.cwd()
...
test('legacy openai saved profiles still deserialize and rebuild startup env', async () => {
  const tempDir = mkdtempSync(join(tmpdir(), 'openairllm-provider-'))
...
test('buildStartupEnvFromProfile preserves plural-profile env when the legacy file is stale', async () => {
  ...
  // addProviderProfile does NOT sync the legacy .openairllm-profile.json,
