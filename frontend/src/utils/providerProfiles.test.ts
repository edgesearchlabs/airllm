...
beforeEach(async () => {
  ...
  testConfigDir = mkdtempSync(join(tmpdir(), 'openairllm-provider-config-'))
...
describe('applyProviderProfileToProcessEnv', () => {
  ...
  test('github-enterprise profile persists and relaunches with Enterprise env', async () => {
    ...
    const configDir = mkdtempSync(join(tmpdir(), 'openairllm-provider-config-'))
    ...
    try {
      ...
      const persisted = JSON.parse(
        readFileSync(join(configDir, '.openairllm-profile.json'), 'utf8'),
      )
...
describe('setActiveProviderProfile', () => {
  ...
  test('persists xAI OAuth profile with marker so logout cleanup can clear it', async () => {
    ...
    try {
      ...
      const profilePath = join(configDir, '.openairllm-profile.json')
  ...
  test('persists no-key openai-compatible profiles for restart fallback', async () => {
    ...
    try {
      ...
      expect(existsSync(join(tempDir, '.openairllm-profile.json'))).toBe(false)
  ...
  test('persists Fireworks AI profiles using a legacy-compatible openai startup profile', async () => {
    const tempDir = mkdtempSync(
      join(tmpdir(), 'openairllm-provider-'),
    )
    const configDir = mkdtempSync(
      join(tmpdir(), 'openairllm-provider-config-'),
    )
    process.chdir(tempDir)
    process.env.CLAUDE_CONFIG_DIR = configDir
    ...
    try {
      ...
      const persisted = JSON.parse(
        readFileSync(
          join(configDir, '.openairllm-profile.json'),
          'utf8',
        ),
      )
      ...
      expect(
        existsSync(join(tempDir, '.openairllm-profile.json')),
      ).toBe(false)
      expect(persisted.profile).toBe('openai')
      expect(persisted.env).toEqual({
