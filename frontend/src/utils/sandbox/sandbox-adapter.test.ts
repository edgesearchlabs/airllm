...
describe('convertToSandboxRuntimeConfig', () => {
  ...
  beforeEach(async () => {
    ...
    tempRoot = await mkdtemp(join(tmpdir(), 'openairllm-sandbox-adapter-'))
  ...
  test('denies canonical OpenAirLLM settings files in changed cwd', () => {
    const config = convertToSandboxRuntimeConfig({} as SettingsJson)

    expect(config.filesystem.denyWrite).toContain(
      resolve(activeCwd, '.openairllm', 'settings.json'),
    )
    expect(config.filesystem.denyWrite).toContain(
      resolve(activeCwd, '.openairllm', 'settings.local.json'),
    )
    expect(config.filesystem.denyWrite).toContain(
