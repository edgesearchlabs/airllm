...
/**
 * How the *currently running* OpenAirLLM installation should be updated.
 *
 ...
 * build the user is running. Custom-PACKAGE_URL builds (OpenAirLLM's
 * `@edgesearch/openairllm`) are safe to self-update.
 *
 * Shared by the `openairllm update` CLI and the `/update` slash command so both
